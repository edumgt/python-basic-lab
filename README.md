...
| **지수와 루트** | `x^2, \sqrt{x}` | $x^2, \sqrt{x}$ |
| **시그마** | `\sum_{i=1}^{n}` | $\sum_{i=1}^{n}$ |
| **그리스 문자** | `\alpha, \beta, \lambda` | $\alpha, \beta, \lambda$ |

---

## 5. 핵심 공식 예시
선형대수에서 가장 기본이 되는 행렬 방정식의 형태입니다.

$$A\mathbf{x} = \mathbf{b}$$

> **Tip:** 마크다운에서 수학 기호를 쓸 때는 반드시 `$` 기호로 감싸야 하며, 복잡한 식은 `$$`를 사용하여 별도의 줄에 작성하는 것이 가독성에 좋습니다.
...
- [`docs/028.md`](docs/028.md): 이차방정식, 미지수, 계수, 상수항, 판별식, 근의 공식
